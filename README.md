...
Player picks are exclusive, and the draft format has been selected with this in mind. Even if you go last, you'll get two picks in a row. Once each team has 5 players, the draft is complete and the league begins! From the moment of your draft completion, every scoring event will add points to the players within your league, and by the end of the season, whichever team has the most points wins! Keep in mind that to begin updating scores your league must all fill out their teams. Should you keep teams empty, it'll effectively act as if you haven't started the league at all.

If you start a league after a couple of events, nothing will be missed. The only caveat being that those past events will not be counted towards you or any of your leaguemates scores.
...
- Trading a player will not get rid of the points they scored previously
- Receiving a player will not grant you the points they scored previously

Let's say two players, `MenaRD` and `Blaz`, score `51` and `43` points respectively from the season start, then a trade window opens. If `MenaRD` is already on your team and you swap him out for `Blaz`, you'll keep those points `MenaRD` scored, but you won't gain those points `Blaz` scored while he was not playing for you. This is of course designed this way in the interest of fairness and to allow strategy. If `Leshar` wins the first CPT event and qualifies for the cup, do you really wanna keep him around?

## **I messed up, and want to change my league after starting it!**

Unfortunately, this app is not built to support rolling back of leagues/teams: what's final is final. If it really bugs you, feel free to create a new login and start a new league there.

## **The scores are incorrect.**

If you happen to catch scores incorrect: fear not. All event standings are managed via the server. That means, regardless what you might see on your app, full control of the season lies within my hands, and so mistakes can be ammended without issue.
...
Nope. Only the windows application currently exists.

## **What data do you store/collect?**

Email addresses and usernames are stored, obviously, to allow logins. Profile pictures will also be visible to me and potentially everyone on the app - so keep it clean. Other than that, no user data is stored/saved (I don't even know how to do that).
